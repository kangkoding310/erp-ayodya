...
import type { Paginated, PurchaseApproval } from '@/types/models';
import { Head, Link } from '@inertiajs/vue3';

defineProps<{
    approvals: Paginated<PurchaseApproval>;
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">My Approvals</h2>
        </template>
                            ...
                            <tbody v-auto-animate class="divide-y divide-gray-100">
                                <tr v-for="approval in approvals.data" :key="approval.id">
                                    <td class="px-4 py-3 text-sm font-medium text-gray-800">
                                        {{ approval.purchase_request?.code }}
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-700">
                                        {{ approval.purchase_request?.employee?.name ?? '-' }}
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-700">
                                        {{ format(approval.purchase_request?.total_amount ?? 0) }}
                                    </td>
